...
An $L_1$ norm is used on each parameter, and the parameters are weighted based on an approximate measure of importance. The total error is calculated as:
\begin{equation}
E = \sum_{i=1}^{N} \| p_i - p_i^{goal} \|_1 *w_i
\end{equation}
where $w_i$ is the scalar importance weighting for the $i$th parameter.
