...
\chapter{Conclusion}
This project investigated using machine learning to make synthesiser programming more efficient and intuitive. 
%A literature review identified several promising techniques for 
An interface has been designed which allows users with no knowledge of synthesiser programming to find new sounds by varying and combining presets. The two novel sections of this interface are the Selection Interface (an interactive low dimensional mapping of a synth preset dataset), and the Blending Interface (an iterative approach to combining presets in order to explore the parameter space). Both of these interfaces use machine learning techniques to extract information from the set of presets.
