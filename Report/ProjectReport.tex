...
In the thesis "Development and Exploration of a Timbre Space Representation of Audio" \ref{SynthTypes} %http://mig.dcs.gla.ac.uk/wp-content/mig-papers/craig_nicol_thesis.pdf
the common methods of synthesiser are decrebed in detail, and are as follows:
\begin{itemize}
...
Talk about automatic tone matching
...
Talk about the Rebecca Frink stuff

\section{Description of HCI design principles for creative musical interfaces}
In the work 'Creativity, Exploration and Control in Musical Parameter Spaces' by Robert H. Tubb. (REFERENCE PROPERLY), the representation and mapping of musical paramers is considered in Section 4.3.
...
\item Many-to-many: a combination of the above (also known as ‘complex’ map- pings).
\end{itemize}
...
ALSO>>>>>Talk about the stuff about multidemsional controlers having higher thoroughput.
...
In Table \ref{tab:EARStable}, the three interfaces are evaluated in terms of the EARS model from (REFERECE)
...
The evaluation of this interface was only carried out on a single synthesiser, due to the project's time constraints. The interface has been designed to be as general purpose as possible, allowing it to control arbitrary synths over the OSC protocol, but due to a lack of standardisation between soft synths, and lack of implementation time, more work needs to be done to create a truly general purpose soft synth controller. As many soft-synths are in the Virtual Studio Technology (VST) format, making a version of the interface with acts as a VST Host, and uses the parameter retrieval and preset storage functionality of VSTs is a good next goal if this project is continued in the future.
