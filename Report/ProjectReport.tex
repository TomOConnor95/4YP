...
A video of the full interface being used can be viewed at (LINK TO VIDEO), and a working demo can be downloaded from (LINK).
...
Synthesisers are devices which can create a wide variety of musical sounds in response to  user input, typically in the form of notes played on a keyboard. %The first synthesisers consisted of single note electronic oscilators, such as the Theremin... , 
...
The DX7 was chosen to as a base synth, as although it is a very powerful, it is notoriously difficult to use due to the phase modulation algorithm being very un-intuitive. An improved interface has the ability to open up FM synthesiser programming to non-expert users. %WORK ON THIS
...
The user is given the option to switch between the Global, and Time/Timbre Macro Controls, as the global controls allow a greater ammount to the space to be searched, but the Time/Timbre controls can be more understandable. See Section \ref{sec:SelectionEval} for a evaluation of the two control types.
