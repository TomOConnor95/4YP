    ...
    methods 
        function obj = generateNewBC(obj)  
            ...
            for i = 1:length(obj.presetA)
                newPresetsMean = (obj.lastValueWeighting*obj.presetA{i} + (1-obj.lastValueWeighting)*weights*obj.presetAHistory{i});

                % Covariance matrix
                sigma = diag(zeros(1,length(obj.presetAHistory{i}(1,:)))+std(obj.presetAHistory{i}).^obj.stdExponent + obj.tempOffset);
